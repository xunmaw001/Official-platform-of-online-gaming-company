...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">公司简介管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>公司简介列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">公司简介列表</h3>							
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th>公司文化</th>
											<th onclick="sort('gongsijieshao')">公司介绍<i id="gongsijieshaoIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var gongsiwenhuaCell  = document.createElement('td');

						  if(item.gongsiwenhua != null && item.gongsiwenhua != "" && item.gongsiwenhua.length >= 11){
			  	gongsiwenhuaCell.innerHTML = item.gongsiwenhua.substring(0, 10) + "..."; 
			  }else{
			  	gongsiwenhuaCell.innerHTML = item.gongsiwenhua;
			  }


			row.appendChild(gongsiwenhuaCell);
			var gongsijieshaoCell  = document.createElement('td');

						  //如果图文详情包含图片
			  if(item.gongsijieshao && item.gongsijieshao.indexOf('img') != -1){
				//暂时只考虑图片+文字 和 文字+图片的情况
				var beginIndex = item.gongsijieshao.indexOf('<img');
				var endIndex = item.gongsijieshao.indexOf('>');				
				gongsijieshaoCell.innerHTML = item.gongsijieshao.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
			  }else{
			  	 if(item.gongsijieshao != null && item.gongsijieshao != "" && item.gongsijieshao.length >= 11){
			  		gongsijieshaoCell.innerHTML = item.gongsijieshao.substring(0, 10) + "..."; 
			  	}else{
			  		gongsijieshaoCell.innerHTML = item.gongsijieshao;
			 	}
			  }
				  


			row.appendChild(gongsijieshaoCell);
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("gongsijianjie/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"gongsijianjie");
