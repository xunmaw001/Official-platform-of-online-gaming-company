    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="youxileixing" id="youxileixing" placeholder="游戏类型" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('youxizhongxin','新增')" @click="jump('../youxizhongxin/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="proBox" v-for="(item,index) in dataList" v-bind:key="index">
        <div class="proBoximg">
          <div class="primg" style="background-size: cover;background-repeat: no-repeat;background-position: center;">
            <img :src="item.youxihaibao?item.youxihaibao.split(',')[0]:''" width="390" height="320" />
          </div>
        </div>
        <div class="proBoxtxt">
          <p>{{item.youximingcheng}}&nbsp;&nbsp;<span style="color: red;" v-if="item.price">{{item.price}} RMB</span></p>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#youximingcheng').val()) {
          param['youximingcheng'] = jquery('#youximingcheng').val() ? '%' + jquery('#youximingcheng').val() + '%' : '';
        }  
        if (jquery('#youxileixing').val()) {
          param['youxileixing'] = jquery('#youxileixing').val() ? '%' + jquery('#youxileixing').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('youxizhongxin/list', 'get', param, function(res) {
