      ...
      <form class="layui-form" style="margin: 10px 0;">
        <div class="layui-inline">
          ...
          <div class="layui-input-inline">
            <input type="text" name="zhaopingangwei" id="zhaopingangwei" placeholder="招聘岗位" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('zhaopinxinxi','新增')" @click="jump('../zhaopinxinxi/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="proBox" v-for="(item,index) in dataList" v-bind:key="index">
        <div class="proBoximg">
          <div class="primg" style="background-size: cover;background-repeat: no-repeat;background-position: center;">
            <img :src="item.tupian?item.tupian.split(',')[0]:''" width="390" height="320" />
          </div>
        </div>
        <div class="proBoxtxt">
          <p>{{item.gongsimingcheng}}&nbsp;&nbsp;<span style="color: red;" v-if="item.price">{{item.price}} RMB</span></p>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#zhaopingangwei').val()) {
          param['zhaopingangwei'] = jquery('#zhaopingangwei').val() ? '%' + jquery('#zhaopingangwei').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('zhaopinxinxi/list', 'get', param, function(res) {
